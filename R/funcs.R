...
runshinySQLite<-function(dbname=''){
  if(dbname!=""){
  library(shiny)
  library(RSQLite)
  library(tidyverse)
  ...
  ui <-navbarPage(title = 'shinySQLitedb',header = '',footer = 
                    navbarPage(title = '',position = 'fixed-bottom',header = '',footer = 
    fluidPage(theme = bs_theme(version = '4',
                 ...
                 tabsetPanel(
                   ...
                   tabPanel('Open/Create.New_db/download_data',
                            tabsetPanel(
                              tabPanel('Open/Create.New_db/',
                                       sidebarLayout(
                                         ...
                                         mainPanel(
                                           ...
                                           dataTableOutput('myavailabledbs'),hr(),'Reads uploaded data',
                                           tableOutput('rid')
                                         ))
                              ),
                              tabPanel('download.data_from db/',
                                       sidebarLayout(
  ...
  server <- function(input, output, session) {
    ...
    output$uplodui<-renderUI({
      if(input$creatoruplod=='uploadData'){
        fluidPage(
          ...
          textInput('saveas','save your file as in the db',placeholder='type the file name here')
        )}
    })
    ...
    observe({
      ...
      invalidateLater(1000, session)
            isolate({redrec(redrec()+1)})
            })
    
    output$rid<-renderTable({
      uploadeddata()[redrec(),]
    })
    ...
    dbtbl2<-reactive({
      ...
      if(
        ...
        if(input$creatoruplod=='uploadData'){
          ...
          dbWriteTable(con, input$saveas, myd, append = F, overwrite=T)
          
        }
        
      }
      
      data <- dbReadTable(con, input$saveas)
    ...
    output$creatdbui<-renderUI({
      fluidPage(
        radioGroupButtons('creatoruplod','create new data or upload data',choices = c('createnewdata','uploadData')),
        uiOutput('editdbname'),
        uiOutput('uplodui'),
    ...
    awesomedt<-reactive({
      ...
      if(input$createnew==T){
        if(input$creatoruplod=='createnewdata'){
          ...
          if(input$creatoruplod=='uploadData'){
            dbWriteTable(con, "awesome", data.frame(dbname=input$saveas,created.on=paste(Sys.time())), append = TRUE)
    ...
    output$downlodui<-renderUI({
      ...
      choices<-read$dbname
      fluidPage(
        selectInput("savethisdb", "Choose a dataset to download:",
                    choices = choices),
        downloadButton("downloaddb", "Download data frame")
      )
    })
    ...
    mydbtosave <- reactive({
      con <- dbConnect(SQLite(), dbname)
      dat<-data.frame(dbReadTable(con,input$savethisdb)
      )
      return(dat)
    })
    ...
    output$downloaddb <- downloadHandler(
      filename = function() {
        paste(paste0(input$savethisdb,".csv"), ".csv", sep = "")
      },
  ...
  # Run the application
  shinyApp(ui = ui, server = server)}else
  {'dbname cannot be empty. Please provide any name you would like use as your database directory'}
